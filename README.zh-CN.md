...
完整的语音记录都会保留下来，而不只是进入步骤的那部分。在指南里打开 **语音记录**，就能看到你说过的全部内容，包括没有匹配到任何步骤的那些，并把其中任意一句加到某个步骤上。编辑步骤也不会毁掉原始口述内容 —— 步骤编辑器始终可以把你真正说过的话放回去。语音记录只留在你的设备上，不会出现在导出的指南里，并会随指南一起删除。
